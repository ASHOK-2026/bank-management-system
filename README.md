# 🏦 Bank and ATM Management System

A C++ console-based Bank and ATM Management System built in *Code::Blocks*, simulating both bank operations and ATM functionalities.

---

## 📌 Project Objectives

This system allows users to:
- Manage bank accounts (open, modify, delete)
- Perform ATM operations (withdrawal, balance inquiry)
- Simulate real-world banking interactions through a menu-driven interface

---

## 🧰 Features

### 🔐 Bank Management:
- Create new account
- View account details
- Modify account information
- Delete account
- List all accounts
- Transfer money
- withdraw money
- deposit money

### 💳 ATM Module:
- PIN verification
- Withdraw cash
- Check available balance
  

---

## 🖥 Built With

- *Language:* C++
- *IDE:* [Code::Blocks](http://www.codeblocks.org/)
- *Concepts Used:*
  - Object-Oriented Programming (OOP)
  - File Handling (Binary/Text files)
  - Menu-driven user interface
  - Data security (simple PIN verification)

---

## 🚀 How to Run the Project in Code::Blocks

1. Open *Code::Blocks*
2. Click on *File → Open* and select the .cpp file (e.g., bank_atm.cpp)
3. Press *F9* or click on *Build and Run*

✅ The terminal-based interface will guide you through banking or ATM operations.

---

